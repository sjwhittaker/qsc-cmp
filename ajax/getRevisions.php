...
if (! $ajax_action) {
    error_log("Couldn't determine the action or type of request in getRevisions.php");
    echo "";
    exit;
}

$query_value = null;
$revisions_array = null;
$db_curriculum = new CMD();
$output_array = array();

if ($ajax_action == QSC_CMP_AJAX_ACTION_SEARCH_REVISIONS) {
    ...
    // Perform the search
    $revisions_array = $db_curriculum->findMatchingRevisions($query_value);
}
else if ($ajax_action == QSC_CMP_AJAX_ACTION_GET_REVISION_FROM_ID) {
    ...
    if (! $query_value) {
        error_log("Couldn't determine the revision's ID in getRevisions.php");
        echo "";
        exit;        
    }
    
    // Get the course
    $revisions_array = array($db_curriculum->getRevisionFromID($query_value));
}

// Go through all the results and create the output with just the IDs
// and names of the revisions
foreach ($revisions_array as $revision) {
    $output_array[] = array(QSC_CMP_AJAX_OUTPUT_ID => $revision->getDBID(),
        QSC_CMP_AJAX_OUTPUT_NAME => $revision->getName(),
        QSC_CMP_AJAX_OUTPUT_LINK => "");
}
