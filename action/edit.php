...
// Check that an 'Edit' form was submitted to get to this page.
...
if (! $js_completed) {
    qsc_core_log_and_display_error("JavaScript was not working properly upon submission of an 'Edit' form. No updates have been made.");    
    header("Location: ".QSC_CMP_ERROR_PAGE_LINK);
    exit();
}

// Determine what type of 'Edit' form was submitted
$redirect_link = null;
$form_result = null;
if ($form_type == QSC_CMP_FORM_TYPE_EDIT_CLLO) {
    // It was an 'Edit CLLO' form
    ...
    $cllo = $db_curriculum->updateCLLOFromPostData($cllo);
    $redirect_link = $cllo->getLinkToView();    
    $form_result = QSC_CMP_FORM_RESULT_EDIT_CLLO_SUCCESSFUL;
}
elseif ($form_type == QSC_CMP_FORM_TYPE_EDIT_PLLO) {
    // It was an 'Edit PLLO' form
    ...
    $pllo = $db_curriculum->updatePLLOFromPostData($pllo);
    $redirect_link = $pllo->getLinkToView();    
    $form_result = QSC_CMP_FORM_RESULT_EDIT_PLLO_SUCCESSFUL;
}

if (! $redirect_link) {
    qsc_core_log_and_display_error("An 'Edit' submission was detected but could not be processed.");
    header("Location: ".QSC_CMP_ERROR_PAGE_LINK);
    exit();
}
?>
